...
impl<'b, F> MarshalFrame<F> for PacketWriter<'b>
where
    BasePacketWriter<'b>: MarshalFrame<F>,
...
impl<'b, F> MarshalPathFrame<F> for PacketWriter<'b>
where
    BasePacketWriter<'b>: MarshalPathFrame<F>,
    for<'f> &'f F: Into<QEventFrame>,
{
    #[inline]
    fn dump_path_frame(&mut self, frame: F) -> Option<F> {
        self.writer.dump_path_frame(frame).map(|frame| {
            self.logger.record_frame(&frame);
            frame
        })
    }
...
impl<'b, F, D> MarshalDataFrame<F, D> for PacketWriter<'b>
where
    BasePacketWriter<'b>: MarshalDataFrame<F, D>,
    D: Clone,
    for<'f> (&'f F, D): Into<QEventFrame>,
{
    #[inline]
    fn dump_frame_with_data(&mut self, frame: F, data: D) -> Option<F> {
        self.writer
            ...
            .map(|f| {
                self.logger.record_frame((&f, data));
                f
            })
    }
}
