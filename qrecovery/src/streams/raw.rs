    ...
    TX: SendFrame<StreamCtlFrame> + Clone + Send + 'static,
{
        ...
        FTX: SendFrame<DataBlockedFrame>,
    {
        ...
        if !self.in_zero_rtt.load(Acquire) {
            return Err(Signals::all()); // load again
        }
        ...
        FTX: SendFrame<DataBlockedFrame>,
    {
        ...
        let (Continue(result) | Break(result)) =
            core::iter::from_fn(|| Some(self.try_load_data_into_once(packet, flow_ctrl))).try_fold(
                Err(Signals::empty()),
    ...
    TX: SendFrame<StreamCtlFrame> + Clone + Send + 'static,
{
    ...
    pub fn revise_params(&self, zero_rtt_rejected: bool, remote_params: &GeneralParameters) {
        if let Ok(output) = self.output.guard() {
            ...
            assert!(self.in_zero_rtt.swap(false, AcqRel));
