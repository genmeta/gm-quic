...
impl PortPredictor {
    ...
    fn port_to_bind_uri(&self, port: u16) -> BindUri {
        ...
        match self.bind_uri.scheme() {
            ...
            BindUriSchema::Inet => {
                ...
                BindUri::from_str(format!("inet://{}:{port}?{TEMPORARY}=true", ip).as_str()).unwrap()
            }
    ...
    pub async fn predict(
        ...
        while rounds_processed < max_rounds && ((self.total_created as u32) < self.max_total) {
            ...
            match self.allocate_and_probe(punch_pair, &packet_send_fn, interfaces_per_round).await {
                Ok(_) => {
                ...
                Err(_) => {
                    last_error = Some(io::Error::other(
                        format!("Failed to process round {}", rounds_processed),
                    ));
                }
            }
            rounds_processed += 1;
    ...
    async fn allocate_and_probe(
            ...
            match self.create_single_interface().await {
                Ok((bind_uri, iface)) => {
                    if let Ok(qbase::net::addr::RealAddr::Internet(socket_addr)) = iface.real_addr() {
                        ...
                        self.ports.push_back((port, bind_uri, iface.clone(), tokio::time::Instant::now()));
        ...
        match self.send_batch_packets(&interfaces, punch_pair, packet_send_fn).await {
            Ok(()) => {
    ...
    async fn send_batch_packets(
        ...
        if successful_sends > 0 || interfaces.is_empty() {
            ...
            Err(io::Error::other(format!("Failed to send packets to all {} interfaces", interfaces.len())))
        }
    }
    ...
    async fn request_quota(&mut self) -> io::Result<()> {
        if self.total_created as u32 + 1 > self.max_total {
            return Err(io::Error::new(
                ...
                format!("Requested 1 port would exceed maximum limit of {}", self.max_total),
            ));
        }
...
impl Drop for PortPredictor {
    fn drop(&mut self) {
        let futures: Vec<_> = self.ports.iter()
