...
use crate::nat::msg::{CHANGE_IP, CHANGE_PORT, Packet};

pub struct Server {
    ifaces: (Arc<StunProtocol>, Arc<StunProtocol>),
    change: SocketAddr,
}

impl Server {
    pub fn new(ifaces: (Arc<StunProtocol>, Arc<StunProtocol>), change: SocketAddr) -> Self {
        info!(
            "new stun io0 {:?}, io1 {:?}",
            ifaces.0.local_addr(),
            ifaces.1.local_addr()
    ...
    pub async fn run(&mut self) -> io::Result<()> {
        info!("Server started");
        let tasks = [
            self.spawn_recv_request_task(self.ifaces.0.clone()),
            self.spawn_recv_request_task(self.ifaces.1.clone()),
        ];
    ...
    fn change_port(&self, protocol: &StunProtocol) -> io::Result<SocketAddr> {
        if protocol.local_addr()? == self.ifaces.0.local_addr()? {
            self.ifaces.1.local_addr()
        } else {
            self.ifaces.0.local_addr()
        }
    }

    fn spawn_recv_request_task(&self, protocol: Arc<StunProtocol>) -> JoinHandle<io::Result<()>> {
        tokio::spawn({
            let local_addr = protocol.local_addr().unwrap();
            let change_port_addr = self.change_port(&protocol).unwrap();
            let change_ip_and_port_addr =
                SocketAddr::new(self.change.ip(), change_port_addr.port());
            async move {
                while let Some((request, txid, src)) = protocol.receive_request().await {
