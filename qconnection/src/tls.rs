...
impl ClientTlsSession {
    ...
    fn try_process_ee(&mut self, parameters: &ArcParameters) -> Result<(), Error> {
        ...
        let handshake_kind = self
            .tls_conn
            .handshake_kind()
            .expect("Handshake kind must be known at this point");
...
impl ArcTlsHandshake {
    ...
    pub fn new(
        ...
        let crypto_read_task = |epoch: Epoch| {
            ...
            async move {
                ...
                while let Ok(read) = stream_reader.read(&mut buf).await {
                    let mut state = tls_handshake.state();
                    let session = &mut state.as_mut().map_err(|e| e.clone())?.session;
                    if let Err(error) = session.write_hs(&buf[..read]) {
                        let error_kind = match session.alert() {
                            Some(alert) => ErrorKind::Crypto(alert.into()),
                            None => ErrorKind::ProtocolViolation,
                        };
                        let reason = format!("TLS error: {error}");
                        return Err(QuicError::with_default_fty(error_kind, reason).into());
                    }
                }
                Result::<_, Error>::Ok(())
            }
        ...
        let crypto_write_task = async move {
            ...
            loop {
                match tls_handshake.read_hs(&mut buf).await? {
                    None if !buf.is_empty() => {
                        // crypto buffer offset overflow
                ...
                tls_handshake.try_process_message(&parameters, &zero_rtt_keys)?;
            }
        };
