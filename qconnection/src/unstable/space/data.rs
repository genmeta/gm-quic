...
use super::{
    super::{
        conn::{CidRegistry, DataStreams, FlowController, Handshake, RcvdPackets},
...
impl DataSpace {
    ...
    pub fn build(
        ...
        let handle_stream_frame_with_flow_ctrl = {
            let flow_ctrl = flow_ctrl.clone();
            let streams = self.streams.clone();
            // 这里需要通过HRTB作为类型提示，否则编译器会推断出一个错误的类型
            // 当然也可以让下面的闭包不需要引用
            fn hrtb_hint<F>(f: F) -> F
            where
                F: for<'a> Fn(&'a (StreamFrame, Bytes)) -> Result<usize, Error>,
            {
                f
            }
            hrtb_hint(move |data_frame| {
                streams.recv_data(data_frame).and_then(|new_data_size| {
                    flow_ctrl.on_new_rcvd(new_data_size).map_err(|e| {
                        Error::new(
        ...
        let dispatch_data_frame = {
            let ccf_broker = event_broker.clone();
            let local_cids_with_router = local_cids_with_router.clone();
            let remote_cid_registry = cid_registry.remote.clone();
            let flow_ctrl_sender = flow_ctrl.sender.clone();
            let flow_ctrl_recver = flow_ctrl.recver.clone();
            let handshake = handshake.clone();
            let crypto_stream_incoming = self.crypto_stream.incoming();
            let streams = self.streams.clone();
            let datagrams = self.datagrams.clone();
            let token_registry = token_registry.clone();

            let inner = move |frame: Frame, pty: Type, path: &Path| match frame {
                Frame::Ack(f) => {
                    path.cc().on_ack(Epoch::Data, &f);
                    on_data_acked(&f);
                    Ok(())
                }
                Frame::NewToken(f) => token_registry.recv_frame(&f),
                Frame::MaxData(f) => flow_ctrl_sender.recv_frame(&f),
                Frame::NewConnectionId(f) => remote_cid_registry.recv_frame(&f).map(|_| ()),
                Frame::RetireConnectionId(f) => local_cids_with_router.recv_frame(&f),
                Frame::HandshakeDone(f) => handshake.recv_frame(&f),
                Frame::DataBlocked(f) => flow_ctrl_recver.recv_frame(&f),
                Frame::Challenge(f) => {
                    path.recv_challenge(f);
                    Ok(())
                }
                Frame::Response(f) => {
                    path.recv_response(f);
                    Ok(())
                }
                Frame::StreamCtl(f) => streams.recv_frame(&f),
                Frame::Stream(f, data) => {
                    handle_stream_frame_with_flow_ctrl(&(f, data)).map(|_| ())
                }
                Frame::Crypto(f, bytes) => crypto_stream_incoming.recv_frame(&(f, bytes)),
                Frame::Datagram(f, data) => datagrams.recv_frame(&(f, data)),
                Frame::Close(f) if matches!(pty, Type::Short(_)) => {
                    ccf_broker.publish(f.into());
                    Ok(())
                }
                _ => Ok(()),
            };

            let error_broker = event_broker.clone();
            move |frame: Frame, pty: Type, path: &Path| {
                if let Err(e) = inner(frame, pty, path) {
                    error_broker.publish(e.into());
                }
            }
        };
                ...
                while let Some((mut packet, pathway, usc)) = any(rcvd_packets.next(), &notify).await
                {
                    ...
                    let undecoded_pn = match remove_protection_of_long_packet(
                        ...
                        Err(invalid_reserved_bits) => {
                            event_broker.publish(invalid_reserved_bits.into());
                            break;
                        }
                    };
                    ...
                    match FrameReader::new(packet.bytes.freeze(), pty).try_fold(
                        ...
                        |is_ack_packet, frame| {
                            ...
                            Result::<_, Error>::Ok(is_ack_packet || is_ack_eliciting)
                        },
                    ) {
                        Ok(is_ack_packet) => {
                        ...
                        Err(e) => event_broker.publish(e.into()),
                    }
                }
                rcvd_packets
            }
